...
jQuery(document).ready(function($){
	...
	-----------------------------------*/
	// tab setup
	$('.top_tab-content').addClass('clearfix').not(':first').hide();
	
	// tab click
	$('ul.top_tabs a[href^="#"]').live('click', function(e){
		e.preventDefault();
		var tabs = $(this).parents('ul.top_tabs').find('li');
		var tabs = $(this).parents('ul.tabs').hide();
		var tab_next = $(this).attr('href');
		var tab_current = tabs.filter('.current').find('a').attr('href');
		$(tab_current).hide();
		tabs.removeClass('current');
 	...
 	// tab hashtag identification and auto-focus
    	...
    	if (wantedTag != "")
    	{
        	var allTabs = $("ul.top_tabs a[href^=" + wantedTag + "]").parents('ul.top_tabs').find('li');
        	var defaultTab = allTabs.filter('.current').find('a').attr('href');
        	$(defaultTab).hide();
        	allTabs.removeClass('current');
        	$("ul.top_tabs a[href^=" + wantedTag + "]").parent().addClass('current');
        	$("#" + wantedTag.replace('#','')).show();
    	}
	/*---------------------------------
		Image Caption
